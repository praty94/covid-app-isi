...
const Sidebar = (props) => {
    ...
    return (
        <div>
            <button onClick={() => toggleDrawer()}>{openState.isOpen ? "Close" : "Open"}</button>
            <div className={openState.isOpen ? styles.mainContainer : cx(styles.mainContainer, styles.displayNone)}>
                <div className={styles.buttonContainer1}>
                    <img src={mainIcon} alt="uppericon" className={styles.img1} />
                    {props.options.map((item,index) => {
                        let classes = null;
                        if (props.selectedPage === item)//if item is selected page then we are adding active class
                            classes = cx(styles.button, styles.active);
                        else
                            classes = styles.button;

                        return <div key={index} className={classes} onClick={() => props.optionSelectedHandler(item)}>{item}</div>;
                    })}
